...
- Agent Desktop allocation now treats VirtualDesktopAccessor numbering as zero-based, permanently reserving native index 0 (Windows UI Desktop 1) while allowing native index 1 (Windows UI Desktop 2) to be used by agents.
- `desktop_start_process` now rejects interactive Chrome/Edge/Chromium and default-browser URL launches. Those launches can hand off to an already-running personal browser process outside DeskMCP ownership, so visible browser automation must use `desktop_browser_session` with an Agent Desktop lease; explicit headless browser CLI remains allowed.
...
- Gateway/runtime regression suite: 185 passed, 0 failed.
